                            ...
                            <form class="needs-validation" id="form1" novalidate method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-row">
                                    <div class="col-md-4 mb-3">
                                        <label for="simple-select2">Products *</label>
                                        <select class="form-control select2" id="simple-select2" name="s_c_id">
                                            <option>Select one</option>
                                            <optgroup label="Products">
                                                <?php
                                                $abc = $pdo->prepare("SELECT * FROM products WHERE p_available='on' && p_qty>=0");
                                                $abc->execute();
                                                $aa = $abc->fetchAll();
                                                foreach ($aa as $aaa) {
                                                    echo '<option data-id="' . $aaa['p_id'] . '" data-price="' . $aaa['p_price'] . '"  data-qty="' . $aaa['p_qty'] . '">' . $aaa['p_name'] . '</option>';
                                                }
                                    ...
                                    <div class="col-md-2 mb-3">
                                        ...
                                        <input class="form-control" type="number" placeholder="0" id="e_qty" required="required" />
                                    </div>
                                    <div class="col-md-2  mb-3">
                                        <label for="e_price">Price</label>
                                        <input class="form-control-plaintext" readonly type="number" placeholder="0" id="e_price" required="required" />
                                    </div>
                                    <div class="col-md-1 mb-3" style="display:flex;justify-content: flex-start;padding: 32px 0px 0px 0px !important;">
                                        <!-- <label for="eadd"> <span class="fe fe-chevrons-down fe-24"></span></label> -->
                                        <button class="btn btn-outline-danger btn-sm" type="button" id="eadd"><i class="fa fa-plus"></i></button>
                                    </div>
                                    <div class="col-md-3  mb-3">
                                        <label for="e_remqty">Remaining Stock</label>
                                        <input class="form-control-plaintext" readonly type="number" placeholder="0" id="e_remqty" required="required" />
                                    </div>
                                </div>
                                <div class="form-row">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align text-danger"><b>Is VAT applicable</b></label>
                                ...
                                <div class="form-row" id="vatdiv">
                                    <label class="col-form-label col-md-3 col-sm-3  label-align text-danger"><b>VAT (13%) </b></label>
                                    <div class="col-md-6 col-sm-6">
                                        <input type="text" readonly name="s_vatorcharge" id="t_vat" value="0" class="form-control-plaintext text-danger">
                                    </div>
                                </div>
                                <div class="form-row">
                                ...
                                <input type="hidden" name="s_uploaded_on" value="<?php echo date("Y-m-d H:i:s"); ?>">
                            ...
                            <form class="needs-validation" id="form3" novalidate method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input class="form-control" id="k" type="file" name="c_photo" />
                                        <div class="valid-feedback"> Looks good! </div>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label for="validationCustom3">Front Citizenship</label>

                                        <input class="form-control" id="l" type="file" name="c_front_citizenship" hidden />
...
<script type="text/javascript">
    ...
    $(document).on("click", "#customCheck1", function() {
        if ($(this).prop("checked") == true) {
            ...
            var vat = (tincome * 0.13).toFixed(2);
            var tamt = Number(vat) + Number(tincome);
            // tamt = tamt.toFixed(2);
    ...
    $("#eadd").on("click", function() {
        var edesc = $("#simple-select2").val();
        var eprice = $("#e_price").val();
        var eqty = $("#e_qty").val();
        var remqty = $("#simple-select2").find(':selected').data('qty');
        var iid = $("#simple-select2").find(':selected').data('id');
        // var remqty = $("#e_remqty").val();
        if (eqty <= remqty) {
            if (eqty == '') {
                eqty = 1;
            }
            ...
            if ($.trim(edesc) == '') {
                $("#simple-select2").css("border", "2px solid red");
            } else {
                $("#simple-select2").css("border", "");
            }
            if ($.trim(eprice) == '') {
            ...
            if ($.trim(edesc) == '' || $.trim(eprice) == '') {
                ...
                tds += '<td><button type="button" name="rem2" class="btn btn-danger btn-sm remove" data-id="' + iid + '"><span><i class="fa fa-times-circle"></i></span></button></td>';
                tds += '</tr>';
                $("#table1").append(tds);
                $("#e_comb").val(ecomb);
                $("#simple-select2").val('');
            ...
            $().msgpopup({
                text: 'Please add more quantity for this product',
                type: 'error'
            });
        }
    });
    ...
    $(document).on('click', '.remove', function() {
        ...
        var tamt = Number(vat) + Number(tot);
        $("#t_vat").val(vat);
        $("#t_amount").val(tamt);
        $("#sal_earnings").val(arr11);
        $.ajax({
