...
<?php
        ...
        $output = '<div class="container-fluid">
              <div class="row justify-content-center">
                <div class="col-12">
                  <h2 class="mb-2 page-title">Loan Transactions Report</h2>
                    ...
                    <div class="col-md-12">
                      ...
                      <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                        <table class="table table-sm table-hover example" id="example">
                            ...
                            <tbody>';
        $daily = date("Y-m-d");
        $dta = $pdo->prepare("SELECT * FROM loan_transactions lt
        INNER JOIN shareholder s ON lt.lt_sh_id=s.s_id
        INNER JOIN admins a ON a.ad_id=lt.lt_u_id WHERE lt.lt_upload_date LIKE '" . $daily . "%'");
        $dta->execute();
        $data = $dta->fetchAll();
        foreach ($data as $dat) {
            ...
            $output .= ' <tr>';
            $output .= '<td><small class="mb-0 text-muted" style="font-size:12px;">' . $dat['lt_upload_date'] . '</small></td>';
            ...
            $output .= '
                                    ...
                                    <td>' . $dat['lt_penalty'] . '</td>
                                    <td>' . $dat['lt_interest'] . '</td>
                                    <td>' . $dat['lt_total'] . '</td>
        ...
        $dta = $pdo->prepare("SELECT * FROM loan_transactions lt
        INNER JOIN shareholder s ON lt.lt_sh_id=s.s_id
        INNER JOIN admins a ON a.ad_id=lt.lt_u_id WHERE lt.lt_upload_date LIKE '" . $month . "%'");
        $dta->execute();
        $data = $dta->fetchAll();
        foreach ($data as $dat) {
        ...
        $dta = $pdo->prepare("SELECT * FROM loan_transactions lt
        INNER JOIN shareholder s ON lt.lt_sh_id=s.s_id
        INNER JOIN admins a ON a.ad_id=lt.lt_u_id WHERE lt.lt_upload_date LIKE '" . $yearly . "%'");
        $dta->execute();
        $data = $dta->fetchAll();
        foreach ($data as $dat) {
